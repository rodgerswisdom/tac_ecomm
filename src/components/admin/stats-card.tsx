...
interface StatsCardProps {
  ...
  icon?: ReactNode
  delay?: number
  /** When set, the whole card becomes a clickable link to this destination */
  href?: string
}
...
export function StatsCard({
  ...
  const cardContent = (
    <Card
      className={cn(
        "relative overflow-hidden border border-brand-teal/10 bg-white shadow-sm transition-all duration-300 rounded-2xl lg:rounded-3xl",
        href && "cursor-pointer group-hover:shadow-xl group-hover:border-brand-teal/30"
      )}
    >
